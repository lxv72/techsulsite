...
// Global variables
	...
	$(function () {
		...
		if (plugins.captcha.length) {
			$.getScript("//www.google.com/recaptcha/api.js?onload=onloadCaptchaCallback&render=explicit&hl=pt");
		}
		...
		function initMaps() {
			...
			$.getScript('//maps.google.com/maps/api/js?' + (key ? 'key=' + key + '&' : '') + 'sensor=false&libraries=geometry,places&v=quarterly', function () {
				...
				for (var i = 0; i < plugins.maps.length; i++) {
					...
					var center = plugins.maps[i].getAttribute("data-center") || "Lisbon";
			...
			for (i = 0; i < plugins.rdMailForm.length; i++) {
				...
				$form.attr('novalidate', 'novalidate').ajaxForm({
					...
					beforeSubmit: function (arr, $form, options) {
						//console.log('RD MAIL FORM BEFORE')
						if (isNoviBuilder)
							return;
					...
					error: function (result) {
						//console.log('RD MAIL FORM ERROR')
						if (isNoviBuilder)
							return;
					...
					success: function (result) {
						//console.log('RD MAIL FORM SUCCESS')
						if (isNoviBuilder)
							return;
